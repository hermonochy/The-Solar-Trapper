...
def main():
    ...
    longitude = -1.2
    elevation = 0
    t = calc(time(), latitude, longitude, elevation, debugtz=timezone(timedelta(hours=0), "fake-zone"))
    print(t)
    currtime = time() -36000
    print(f"Current time {_ts2human(currtime,None)}")
    print(f"Sunrise before {(currtime-t[0])/3600} hours")
    print(f"Sunset in {(t[1]-currtime)/3600} hours")
