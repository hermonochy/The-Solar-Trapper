...
The objectives of this project have been broken down into the following:

\begin{enumerate}
    \item Build a system which helps to save energy by reducing the need for space heating. Success criterion: The room is at its peak temperature when the owner returns home.
    \item The system should be capable of running automatically while people are away for the day. Success criterion: No user interaction is required during the day.
    \item The system should be affordable. Success criterion: Hardware costs do not exceed £50. No cost for software.
    \item The system should be low cost to run. Success criterion: Cost at most £0.1 per day.
\end{enumerate}
...
This system does not have many safety or ethical issues, as hardware played a relatively minor part in this project.

\subsubsection{Safety}

\begin{itemize}
    \item Connections are not insulated, creating a risk of short circuits. Therefore, the prototype has to be supervised while in use.
    \item The curtain system does not have an IP code, so it is limited to indoor use. 
    \item The hot glue gun had to be handled carefully to avoid burns.
    \item A teddy bear was used as the gravitational potential energy store to avoid any injuries when being released.
\end{itemize}

\subsubsection{Ethics}
    
    Ethics do not apply to this project, as no humans or animals were used in the making of this system.
...
Table \ref{tab:equipment} lists the parts that were required while building this project. This shows this design to be a very economic solution.
...
\begin{table}[h!]
    ...
    \begin{tabular}{l | l | l}
        ...
        Servo Motor & £ 3.99 & Scavenged from an old project \\
...
However, as most of the costs come from the model parts, the cost of the full sized version would likely be an estimated £30, 
affordable by most people. A more realistic table may look like Table \ref{tab:realisticEquipment}.

\begin{table}[h!]
    \caption{Actual Equipment}
    \begin{tabular}{l | l | l}
        Parts & Price\\
        \hline
        LDR Photo Resistor & £ 3.99\\
...
with open(device_file, 'r') as f, open(date+'.csv', 'w') as outputfile, 
               serial.Serial('/dev/ttyUSB0', 9600, timeout = 2) as ser:
  while True:
    lines = f.readlines()
...
The system was tested over the period of a week, and the results proved promising. If additional tests continue to report positive outcomes, this project could solve
the heating crisis not only in the UK, but also other low-temperature regions, such as Eastern Europe\cite{url:EasternEuropeDeathsFromCold}. 
With an average UK heating bill of roughly £100\cite{url:heatingBillsUK} per month, The Solar Trapper already pays for itself within half a month.
...
\begin{itemize}
    \item Using GPIO's on a Rasberry Pi
    \item Using Matplotlib and Pandas
    \item Using Latex
    \item Data logging
...
\begin{thebibliography}{99}
    ...
    \bibitem{url:repo} The Smart Solar Trap. \url{https://github.com/hermonochy/The-Solar-Trapper}
    ...
    \bibitem{url:1wire} Activating the 1-wire protocol on Raspbian: 
        \url{https://www.raspberrypi-spy.co.uk/2018/02/enable-1-wire-interface-raspberry-pi/}, accessed on 27 Feb 2025.

    \bibitem{url:digitalTempSensor} DS18B20 Sensor: \url{https://randomnerdtutorials.com/raspberry-pi-ds18b20-python/}

    \bibitem{url:sunsetEquation} Sunset Equation: \url{https://en.wikipedia.org/wiki/Sunrise_equation}

    \bibitem{url:EasternEuropeDeathsFromCold} "Hundreds die from the cold in eastern Europe": \url{https://www.wsws.org/en/articles/2012/02/cold-f03.html}

    \bibitem{url:heatingBillsUK} Heating Bills in the UK: \url{https://plumr.co.uk/uk-heating-bill-estimator-tool/}

    \bibitem{url:openmeteo} Open Meteo: \url{https://open-meteo.com/}.
